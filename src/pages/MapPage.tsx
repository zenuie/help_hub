...
export default function MapPage() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribeToMarkers((markersFromDb) => {
            console.log('[markersFromDb]', markersFromDb.length, markersFromDb);
        ...
        map.on('load', () => {
            ...
            setTimeout(() => {
                if (!selectedPlace && allMarkers.length > 0) {
                    ...
                    console.log('[on load flyTo first]', m.id, m.lng, m.lat);
    ...
    useEffect(() => {
        ...
        allMarkers.forEach(markerData => {
            if (!markerIndex.has(markerData.id)) {
                console.log('[createMarkerOnMap]', markerData.id, markerData.lng, markerData.lat);
        ...
        markerIndex.forEach(entry => {
            ...
            console.log('[shouldShow]', entry.data.id, show, selectedPlace);
            entry.obj.getElement().style.display = ''; // 暫時全部顯示
        });

        if (allMarkers.length > 0) {
            const latest = [...allMarkers].sort((a, b) => b.updatedAt.toMillis() - a.updatedAt.toMillis())[0];
            console.log('[flyTo latest]', latest.id, latest.lng, latest.lat);
    ...
    return (
        <div style={{padding: 8}}>
            {isModalOpen && (<div style={modalOverlayStyle}>
                <div style={modalContentStyle}><h2>新增標註點</h2><p
                    style={{fontSize: 14, color: '#666', marginTop: 0}}>座標: {newItemForm.lat?.toFixed(5)}, {newItemForm.lng?.toFixed(5)}</p><label>類別：</label><select
                    value={newItemForm.type} onChange={e => setNewItemForm(p => ({...p, type: e.target.value as MarkerType}))}
                    style={{width: '100%', padding: 8, fontSize: 16}}>{Object.entries(enToZh).map(([en, zh]) => <option key={en} value={en}>{zh}</option>)}</select><label
                    style={{marginTop: 12}}>簡要描述：</label><textarea value={newItemForm.description ?? ''}
                    ...
                    <div style={{marginTop: 20, display: 'flex', justifyContent: 'flex-end', gap: 12}}>
                        <button onClick={() => setIsModalOpen(false)}
                                style={{background: '#eee', border: '1px solid #ccc', padding: '8px 16px', borderRadius: 6, cursor: 'pointer'}}>取消
                        </button>
            ...
            <div style={{marginTop: 8}}>
                <button onClick={() => setFilterOpen(v => !v)} style={filterToggleStyle}>{filterOpen ? '收合地區篩選 ▲' : '展開地區篩選 ▼'}</button>
                {filterOpen && (<div style={filterPanelStyle}>
                    <div style={{display: 'flex', gap: 8}}><input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="輸入鄉鎮市區，如：光復鄉"
                                                                  style={searchInputStyle}/>
                        <button onClick={() => searchPlace(searchInput)} disabled={isSearching} style={searchBtnStyle}>{isSearching ? '搜尋中…' : '搜尋'}</button>
                    </div>
                    {searchResults.length > 0 && (
                        <ul style={{marginTop: 8, display: 'grid', gap: 8, maxHeight: 240, overflow: 'auto', padding: 0, listStyle: 'none'}}>{searchResults.map((r, idx) => <li
                            key={idx}>
                            <button onClick={() => onPickSearchResult(r)} style={bigListBtnStyle}>{r.label}</button>
                        </li>)}</ul>)}{customPlaces.length > 0 && (<>
                    <div style={{fontWeight: 600, marginTop: 12, marginBottom: 6}}>常用地區</div>
                    <ul style={{display: 'grid', gap: 8, padding: 0, listStyle: 'none'}}>{customPlaces.map(p => (<li key={p.id || p.label}>
                        <div style={{display: 'flex', gap: 8}}>
                            <button onClick={() => {
                                applyPlace(p);
                                saveLastPlace(p);
                            }} style={bigListBtnStyle}>{p.label}</button>
                            <button onClick={() => removeCustom(p.id)} style={{...bigListBtnStyle, width: 'auto', flexShrink: 0}}>刪除</button>
                        </div>
                    </li>))}</ul>
                </>)}
                    <div style={{marginTop: 8}}>
            ...
            <div id="map" style={{height: `${Math.max(400, vh - 400)}px`, border: '1px solid #ccc', borderRadius: 12, marginTop: 8, position: 'relative'}}/>

            <div style={{marginTop: 8}}>
                <div style={{fontWeight: 600, marginBottom: 6}}>標註列表 ({selectedPlace ? `篩選：${selectedPlace.label}` : '未篩選'})</div>
                {isDataLoading ? <p>正在載入標註資料...</p> : (<ul style={{display: 'grid', gap: 8, padding: 0, listStyle: 'none'}}>{visibleEntries.map((data) => {
                    const zhLabel = enToZh[data.type];
                    const isLinked = !!data.linkedTaskId;
                    return (<li key={data.id} style={{display: 'flex', gap: 8, alignItems: 'center'}}>
                        <button onClick={() => handleRemoveMarker(data.id)} title="刪除此標註"
                        ...
                        <div style={{padding: '8px 10px', border: '1px solid #ddd', borderRadius: 8, flex: 1}}>
                            <div style={{fontWeight: 600}}><span style={{
                            ...
                            }}/>{zhLabel}{isLinked && <span style={{marginLeft: 8, fontSize: 12, color: '#10b981'}}>(已轉任務)</span>}</div>
                            <div style={{
                                color: '#6b7280',
                                fontSize: 13,
                                marginTop: 4
                            }}>{data.fullAddress ? `地址：${data.fullAddress}` : `${formatAdmin(data.city, data.district)}／座標：${formatCoord(data.lat, data.lng)}`}</div>
                        </div>
                        <button onClick={() => handleCreateTaskFromMarker(data)} disabled={isLinked} title={isLinked ? `已轉為任務` : "以此標註為地點建立任務"} style={{
                            padding: '8px 10px',
